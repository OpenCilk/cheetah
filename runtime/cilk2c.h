...
CHEETAH_API void __cilkrts_check_exception_raise(struct __cilkrts_stack_frame *sf);

// Check if the runtime is storing an exception we need to handle later, and
// resumes unwinding with that exception if so.
CHEETAH_API void __cilkrts_check_exception_resume(struct __cilkrts_stack_frame *sf);

// Performs runtime operations to handle a cilk_sync.
__attribute__((noreturn, nothrow))
CHEETAH_API void __cilkrts_sync(struct __cilkrts_stack_frame *sf);
...
CHEETAH_API void __cilkrts_leave_frame(struct __cilkrts_stack_frame *sf);
...
CHEETAH_API
void __cilkrts_cleanup_fiber(struct __cilkrts_stack_frame *, int32_t sel);

// Not marked as CHEETAH_API as it may be deprecated soon
unsigned __cilkrts_get_nworkers(void);

CHEETAH_API
void __cilkrts_set_return(struct __cilkrts_worker *const ws);
CHEETAH_API
void __cilkrts_exception_handler(struct __cilkrts_worker *w, char *exn);
