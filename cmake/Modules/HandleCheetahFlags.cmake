...
macro(config_define_if condition def)
  if (${condition})
    set(${def} ON)
    set(CHEETAH_NEEDS_SITE_CONFIG ON)
  endif()
endmacro()
...
# 'CHEETAH_COMPILE_FLAGS' and 'CHEETAH_LINK_FLAGS'.
macro(add_target_flags)
  foreach(value ${ARGN})
    set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} ${value}")
    set(CMAKE_C_FLAGS "${CMAKE_C_FLAGS} ${value}")
    list(APPEND CHEETAH_COMPILE_FLAGS ${value})
    list(APPEND CHEETAH_LINK_FLAGS ${value})
  endforeach()
endmacro()

# If the specified 'condition' is true then add a list of flags to
# all of 'CMAKE_CXX_FLAGS', 'CMAKE_C_FLAGS', 'CHEETAH_COMPILE_FLAGS'
# and 'CHEETAH_LINK_FLAGS'.
...
# Add a specified list of flags to both 'CHEETAH_COMPILE_FLAGS' and
# 'CHEETAH_LINK_FLAGS'.
macro(add_flags)
  foreach(value ${ARGN})
    list(APPEND CHEETAH_COMPILE_FLAGS ${value})
...
# Add each flag in the list to CHEETAH_COMPILE_FLAGS and CHEETAH_LINK_FLAGS
# if that flag is supported by the current compiler.
macro(add_flags_if_supported)
  foreach(flag ${ARGN})
      mangle_name("${flag}" flagname)
      check_cxx_compiler_flag("${flag}" "CHEETAH_SUPPORTS_${flagname}_FLAG")
      add_flags_if(CHEETAH_SUPPORTS_${flagname}_FLAG ${flag})
  endforeach()
endmacro()

# Add a list of flags to 'CHEETAH_COMPILE_FLAGS'.
macro(add_compile_flags)
  foreach(f ${ARGN})
    list(APPEND CHEETAH_COMPILE_FLAGS ${f})
  endforeach()
endmacro()

# If 'condition' is true then add the specified list of flags to
# 'CHEETAH_COMPILE_FLAGS'
...
# For each specified flag, add that flag to 'CHEETAH_COMPILE_FLAGS' if the
# flag is supported by the C++ compiler.
macro(add_compile_flags_if_supported)
  foreach(flag ${ARGN})
      mangle_name("${flag}" flagname)
      check_cxx_compiler_flag("${flag}" "CHEETAH_SUPPORTS_${flagname}_FLAG")
      add_compile_flags_if(CHEETAH_SUPPORTS_${flagname}_FLAG ${flag})
  endforeach()
endmacro()
...
macro(add_c_compile_flags_if_supported)
  foreach(flag ${ARGN})
      mangle_name("${flag}" flagname)
      check_c_compiler_flag("${flag}" "CHEETAH_SUPPORTS_${flagname}_FLAG")
      add_compile_flags_if(CHEETAH_SUPPORTS_${flagname}_FLAG ${flag})
  endforeach()
endmacro()

# Add a list of flags to 'CHEETAH_LINK_FLAGS'.
macro(add_link_flags)
  foreach(f ${ARGN})
    list(APPEND CHEETAH_LINK_FLAGS ${f})
  endforeach()
endmacro()
...
# For each specified flag, add that flag to 'CHEETAH_LINK_FLAGS' if the
# flag is supported by the C++ compiler.
